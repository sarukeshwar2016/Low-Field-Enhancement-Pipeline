# ============================================================
# FINAL: RESEARCH-GRADE MRI ENHANCEMENT PIPELINE (BATCH 61-112)
# ============================================================
...
def process_patient(lf_path, hf_path):
    ...
    if compute_snr(w_out) > hf_snr:
        w_out *= hf_snr / (compute_snr(w_out) + 1e-8)
    ...
    hf_aligned = np.transpose(sitk.GetArrayFromImage(resampler.Execute(hf_sitk)), (2,1,0)).astype(np.float32)
...
# ============================================================
def main():
    # 1. Load existing report details (1-60)
    report_header = "MRI ENHANCEMENT (BATCH 1-112)\n" + "="*50 + "\n"
    report_details = ""
    summary = []
    ...
    if os.path.exists(REPORT_PATH):
        with open(REPORT_PATH, "r") as f:
            lines = f.readlines()
            for line in lines:
                if "FINAL SUMMARY" in line: break
                if "NEW BATCH ENHANCEMENT" in line: continue
                if "======" in line and len(line.strip()) > 40: continue # Header separator
                report_details += line
                
                # Extract summary stats for existing entries
    ...
    # 2. Identify files to process (61-112)
    lf_files = sorted(glob.glob(os.path.join(LF_DIR, "*_LF.nii.gz")))
    target_files = [f for f in lf_files if 61 <= int(os.path.basename(f)[:4]) <= 112]
    
    print(f"Enhancing {len(target_files)} patients (61-112)")

    for lf_path in target_files:
        pid = os.path.basename(lf_path)[:4]
        # Match with T2SAG format HF
        hf_path = os.path.join(HF_DIR, f"{pid}_T2SAG.nii.gz")
        if not os.path.exists(hf_path): continue

        chunk, stats = process_patient(lf_path, hf_path)
        report_details += chunk
    ...
    for s in summary:
        summary_text += f"{s['id']} | SNR={s['snr']:.2f} | PSNR={s['psnr']:.2f} | SSIM={s['ssim']:.4f}\n"

    with open(REPORT_PATH, "w") as f:
        f.write(report_details + summary_text)
