# ============================================================
# FINAL: RESEARCH-GRADE MRI ENHANCEMENT PIPELINE (50 NEW BATCH)
# ============================================================
...
def process_patient(lf_path, hf_path):
    ...
    hf = hf_nii.get_fdata().astype(np.float32) # (X, Y, Z)
    ...
    # STAGE 1: N4
    # ============================================================
    # ✅ FIX: Transpose for SITK (expecting ZYX)
    ...
    if snr_f < 0.7 * hf_snr:
        final *= (0.7 * hf_snr)/(snr_f + 1e-8)
    elif snr_f > 0.9 * hf_snr:
        final *= (0.9 * hf_snr)/(snr_f + 1e-8)
...
# ============================================================
def main():
    lf_files = sorted(glob.glob(os.path.join(LF_DIR, "*_LF.nii.gz")))[:50]
    print(f"Found {len(lf_files)} simulated volumes. Starting enhancement batch...")
    
    report = "NEW BATCH ENHANCEMENT (50 PATIENTS)\n" + "="*50 + "\n"
    summary = []
    ...
    for lf_path in lf_files:
        ...
        if not os.path.exists(hf_path):
            print(f"Skipping {pid}: HF reference not found at {hf_path}")
            continue
