...
Converts high-field MRI DICOM to NIfTI and applies physics-realistic
low-field simulation: voxel resampling, PSF blur, Rician noise injection,
and strict SNR enforcement (LF SNR always < HF SNR).
...
def simulate_low_field(hf_path, out_dir, patient_id):
    ...
    try:
        ...
        # Scale to match original HF mean within Â±5%
        ...
        if final_lf_snr >= hf_snr:
            print(f"    âŒ Rejecting {patient_id}: LF SNR ({final_lf_snr:.2f}) >= HF SNR ({hf_snr:.2f})")
            return None
        ...
        print(f"    âœ… Saved: {os.path.basename(final_path)}")

        return final_path

    except Exception as e:
        print(f"    âš ï¸ ERROR in {patient_id}: {e}")
        return None
